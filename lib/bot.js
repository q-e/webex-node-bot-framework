...
Bot.prototype.uploadStream = function (filename, stream) {
  ...
  if (typeof filename === 'string' && (stream instanceof Stream || stream instanceof Buffer)) {
...
Bot.prototype.messageStreamRoom = function (roomId, message) {
  ...
  const messages = {};
  messages.roomId = this.room.id;
  messages.file = requestOptions.formData.files;
  return this.framework.webex.messages.create(messages);
};
